...
| 2026-09-20 11:22 UTC | same, **V14** (closed form, once per row) | **7,302** | 7,300 | 3,504 | 4,471 | **42.0 %** | ❌ still. Halved across the board; Postgres still 82–84 % for the whole window with every app idle. One bottleneck gone, another underneath it → measured with `EXPLAIN ANALYZE` on the estate: 5,000 rows cost 309 ms through the view against a 9 ms floor, all of it the function boundary → V15 |
| 2026-09-20 11:41 UTC | same, **V15** (variance inline, calendar joined once) | 1,689 | 802 | **224** ✅ | 508 | **0 %** ✅ | ❌ tree (p95 < 1 s), summary (< 300 ms), write (< 500 ms, by 8 ms). **Nothing 5xx'd and nothing was rate-limited**: the breaker stayed shut, which is the difference between "slow" and "broken". Postgres peaked at **66 %**, no longer pinned. p50s: tree 1,184 ms, summary 319 ms, detail 139 ms, write 166 ms — a median PM open is now a second, and the p95 is the 25-VU ramp on one burstable vCPU |

**Where that leaves the promise.** The thresholds were written before anything had been measured, for "a PM opens a 5,000-row board in a second". On a B1ms — one burstable vCPU, shared with the write path and the outbox relay — 25 concurrent PMs reading 5,000 rows each is 125,000 rows a pass, and the remaining gap is queueing, not a query: successful responses no longer fail, they wait. The next move is a decision, not a fix: either the thresholds say "on the pilot's hardware" and are re-stated against a measured B2s/B4ms, or the dev estate is sized up for the test and the numbers mean what they say. Both are H4. What is *not* left is a code finding — the two the test produced (V14, V15) are in and the DB is off the floor.
