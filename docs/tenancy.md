...
✅ **The platform is single-tenant, and that single tenancy is enforced at five places, not
assumed.** All five services pin `issuer-uri` to one Entra tenant (`template-service` and
`integration-service` carried the guard from their first commits):
...
`milestone-service`, at `identity-service`, at `template-service` and at `integration-service`, before any handler runs. **There is no code path
